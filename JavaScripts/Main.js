...
onload = function () {
    ...
    this.document.getElementById("button_left").onmousedown = function () {
        window.onkeydown({ code: "ArrowLeft" });
    }
    this.document.getElementById("button_left").addEventListener('touchstart', () => {
        window.onkeydown({ code: "ArrowLeft" })
    })
    this.document.getElementById("button_left").onmouseup = function () {
        window.onkeyup({ code: "ArrowLeft" });
    }
    this.document.getElementById("button_left").addEventListener('touchend', () => {
        window.onkeyup({ code: "ArrowLeft" })
    })

    this.document.getElementById("button_right").onmousedown = function () {
        window.onkeydown({ code: "ArrowRight" });
    }
    this.document.getElementById("button_right").addEventListener('touchstart', () => {
        window.onkeydown({ code: "ArrowRight" })
    })
    this.document.getElementById("button_right").onmouseup = function () {
        window.onkeyup({ code: "ArrowRight" });
    }
    this.document.getElementById("button_right").addEventListener('touchend', () => {
        window.onkeyup({ code: "ArrowRight" })
    })

    this.document.getElementById("button_enter").onmousedown = function () {
        window.onkeydown({ code: "Enter" });
    }
    this.document.getElementById("button_enter").addEventListener('touchstart', () => {
        window.onkeydown({ code: "Enter" })
    })
    this.document.getElementById("button_enter").onmouseup = function () {
        window.onkeyup({ code: "Enter" });
    }
    this.document.getElementById("button_enter").addEventListener('touchend', () => {
        window.onkeyup({ code: "Enter" })
    })

    this.document.getElementById("button_space").onmousedown = function () {
        window.onkeydown({ code: "Space" });
    }
    this.document.getElementById("button_space").addEventListener('touchstart', () => {
        window.onkeydown({ code: "Space" })
    })
    this.document.getElementById("button_space").onmouseup = function () {
        window.onkeyup({ code: "Space" });
    }
    this.document.getElementById("button_space").addEventListener('touchend', () => {
        window.onkeyup({ code: "Space" })
    })
};
...
class GameOverState extends GameState {
    ...
    update() {
        ...
        if (!notificationCentre.getIsDawned()) {
            ...
            ctx.font = "18px 'Times New Roman'";
            ctx.fillText(this.clearTimeText, 30, 30);
        }
    }
}